...
//  2) { resultContent } 전달 -> 신청자 본인의 출장결과 제출
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.businessTripRequest.findUnique({ where: { id } });
    if (!existing) {
    ...
    if (resultContent !== undefined) {
      ...
      if (existing.status !== "APPROVED") {
        return NextResponse.json({ error: "승인된 출장신청에만 결과를 제출할 수 있습니다." }, { status: 400 });
      }
      const trimmed = String(resultContent).trim();
      if (!trimmed) {
        return NextResponse.json({ error: "출장결과 내용을 입력해 주세요." }, { status: 400 });
      }
